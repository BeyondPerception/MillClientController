...
/**
 * A wrapper connecting a GStreamer AppSink and a JavaFX Image
 * <p>
...
public class FXImageSink {
	 ...
	 * Image should only be accessed on the JavaFX application thread. Use of the
	 * Image when it is no longer the current value of this property may cause
	 * errors or crashes.
	 *
	...
	/**
	 * Clear any image and dispose of underlying native buffers. Can be called from
	 * any thread, but clearing will happen asynchronously if not called on JavaFX
	 * application thread.
	 */
	public void clear() {
	...
	/**
	 * Request the given frame size for each video frame. This will set up the Caps
	 * on the wrapped AppSink. Values are in pixels. A value of zero or less will
	 ...
	 * AppSink. Value is in frames per second. A value of zero or less will result
	 * in the value being omitted from the Caps.
	 *
	...
	private WritableImage convertBytesToImage(ByteBuffer pixels, int width, int height) {
		// Writes a ByteBuffer to a WritableImage.
	...
	private void updateImage() {
		...
		WritableImage img = convertBytesToImage(activeBuffer.map(false), width, height);
	...
	private class NewSampleListener implements AppSink.NEW_SAMPLE {
		...
		public FlowReturn newSample(AppSink appsink) {
			...
			if (s != null) {
				// if not null the Sample has not been taken by the application thread so
				// dispose
				s.dispose();
			}
